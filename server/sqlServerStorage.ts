...
export async function findInquiryByContactPhone(contactNum: string) {
  try {
  ...
  try {
    ...
    try {
      ...
      if (result.recordsets && result.recordsets.length > 1) {
        ...
        const attendance = parseFloat(balanceData['AttendancePresent'] || '0') || 0.0;
        const absences = parseFloat(balanceData['UnexcusedAbsences'] || '0') || 0.0;
        const adjustments = parseFloat(balanceData['MiscAdjustments'] || '0') || 0.0;
...
export async function searchStudent(contactNum: string) {
  try {
    // Step 1: Lookup parent/inquiry info using contact number
    const inquiry = await findInquiryByContactPhone(contactNum);
    if (!inquiry) {
